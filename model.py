...
class ReplayMemory(object):
    def __init__(self, capacity, state_shape, n_actions, device):
        ...
        self.device = device
        self.m_states = np.zeros((capacity, c, h, w), dtype=np.uint8)
        self.m_actions = np.zeros((capacity, 1), dtype=np.int8)
        self.m_rewards = np.zeros((capacity, 1), dtype=np.int8)
        self.m_dones = np.zeros((capacity, 1), dtype=np.bool)
        self.position = 0
        self.size = 0
    ...
    def sample(self, bs):
        ...
        bs = torch.from_numpy(self.m_states[i, :4]).to(self.device)
        bns = torch.from_numpy(self.m_states[i, 1:]).to(self.device)
        ba = torch.from_numpy(self.m_actions[i]).to(self.device).long()
        br = torch.from_numpy(self.m_rewards[i]).to(self.device).float()
        bd = torch.from_numpy(self.m_dones[i]).to(self.device).float()
